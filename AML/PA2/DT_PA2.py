...
class Skeleton:
    ...
    def get_skeleton(df):
        ...
        for col in df.columns:
            skeleton[i] = df[col].unique()
            i += 1
        return skeleton

    @staticmethod
    def set_feature_count(df):
        Skeleton.num_features = len(df.columns)

    @staticmethod
    def get_feature_count():
        return Skeleton.num_features
...
def read_data(filename):
    ...
    df_temp.columns = ["droppable", "cap-shape", "cap-surface", "cap-color", "bruises", "odor", "gill-attachment",
                       "gill-spacing", "gill-size", "gill-color", "stalk-shape", "stalk-root",
                       "stalk-surface-above-ring",
                       "stalk-surface-below-ring", "stalk-color-above-ring", "stalk-color-below-ring", "veil-type",
                       "veil-color", "ring-number", "ring-type", "spore-print-color", "population", "habitat"]

    df_temp.drop('droppable', 1, inplace=True)
    bruises = df_temp['bruises']
    df_temp.drop('bruises', 1, inplace=True)
    df_temp.insert(0, 'bruises', bruises)
    Skeleton.skeleton = Skeleton.get_skeleton(df_temp)
    Skeleton.set_feature_count(df_temp)
...
def filter_data(data, mask):
    ...
    Means we want to filter data so that it only has a value of 2 for the 3rd
    feature.
...
class DecisionTreeNode:
    ...
    def __init__(self):
        DecisionTreeNode.num_features = 21
        self.idx = -1
        self.data = []
    ...
    def get_children(self):
        ...
        """
        children = []
        for val in Skeleton.skeleton.get(self.idx):
...
if __name__ == '__main__':
    data_train1 = read_data("mushroom_data.csv")
